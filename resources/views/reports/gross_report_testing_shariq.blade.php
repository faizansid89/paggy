 ...
 */
                                                        ...
                                                        $gross = 0;
                                                        $costPrice = getMedicineCostPrice($todaySale->sku, $todaySale->batch_code, $todaySale);
                                                        //if($costPrice==0){
                                                    @endphp
                                                    <tr id="rowID-{{ $todaySale->product_id }}">
                                                        <td>
                                                            <a href="{{ route("products.show", $todaySale->product_id) }}">{{$todaySale->product_name}}</a>
                                                        </td>
                                                        <td>{{$todaySale->quantity}} {{ $todaySale->unit_name }}
                                                            x {{number_format($todaySale->price, 2)}}
                                                            = {{ number_format($todaySale->quantity * $todaySale->price, 2) }}</td>
                                                        <td>{{$todaySale->discount}}</td>
                                                        <td>{{$todaySale->sub_total}}</td>
                                                        <td>
                                                            @php

                                                                $gross = $todaySale->sub_total - $costPrice;
                                                                $grossProfit += $gross;
                                                            @endphp
